...
#
# regular (default): ctest -LE "stress|soak"  -- every PR
# stress:            ctest -L  stress          -- nightly (docs/15 §12.1)
# soak:              ctest -L  soak            -- nightly 1h ge_soak lane (GE_SOAK_DURATION, docs/15 §11)
#
# Environment:
...
#   GE_SOAK_DURATION  ge_soak run length for the soak suite (default 1h)
set -eu
...
soak_flags=""
if [ "$suite" = "soak" ]; then soak_flags="-DGE_SOAK_DURATION=${GE_SOAK_DURATION:-1h}"; fi
...
cmake -S "$root" -B "$build" -G "$generator" $flags $stress_flags $soak_flags -DGE_ENABLE_MEDIA=ON -DGE_ENABLE_ONNX=ON
cmake --build "$build" -j "$jobs"

cd "$build"
case "$suite" in
  regular) ctest --output-on-failure -LE "stress|soak" -j 4 ;;
  stress)  ctest --output-on-failure -L stress ;;
  soak)    ctest --output-on-failure -L soak ;;
  *) echo "unknown suite '$suite' (regular|stress|soak)" >&2; exit 2 ;;
esac
